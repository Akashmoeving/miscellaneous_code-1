...
conn = mysql.connector.connect(host="localhost:3306", user="live_user", password="1PC^&uO$5",
                               database="mydb")
cursor = conn.cursor()
bearer_token = ('eyJ0eXAiOiJKV1QiLCJhbGciOiJIUz')
...
def update_function(pan, user_id):
    ...
    if (aadhar_pan_link_response['status'] == 'success' and
        ...
        if aadhar_linking_status == True:
            ...
            update_user_aadhar_mismatched = """UPDATE moevingdb.pan_detail SET 
                                                                                pan = %s,
...
def insert_user_aadhar_pan_linking(pan_number, user_id):
    try:
        ...
        if (aadhar_pan_link_response['status'] == 'success' and
            ...
            insert_unverified_pan = """INSERT INTO moevingdb.pan_detail(created_by,user_id, remarks)
                                        VALUES( %s, %s, %s);"""
