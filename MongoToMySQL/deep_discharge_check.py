...
def insert_to_mysql_table():
    truncate_table = "TRUNCATE moevingdb.vehicle_soc_data "
...
def post_to_webhook(bot_msg):
    ...
    # Test
    # url = (
    #     'https://chat.googleapis.com/v1/spaces/AAAAQqFHSzs/messages?key=AIzaSyDdI0hCZtE6vySjMm-WEfRq3CPzqKqqsHI&token=ObHS4RWRvTHdYPm7JR_o6pkfayQD_SGcAHzDcbj0Rtk')

    # Prod
    url = (
        'https://chat.googleapis.com/v1/spaces/AAAAKqbLC0g/messages?key=-WEfRq3CPzqKqqsHI&token=RFAWBu3kmrLlG8UTakYW2P_53ATOyOAuNibEtNU6I7M')
...
def select_from_deep_discharge():
    from_discharge_data = ("select vehicle_number,soc_threshold_count from "
                           "moevingdb.vehicle_deep_discharge_details")
...
def select_from_demand_fulfillment():
    demand_data = ("select tvd.FLD_RC_NUMBER , date(FLD_DRIVER_CHECKIN_TIME) from "
                   ...
                   " moevingdb.tvd.FLD_VEHICLE_ID  = tdfdd.FLD_VEHICLE_ID where date(FLD_DRIVER_CHECKIN_TIME) = "
                   " CURRENT_DATE();")
...
def call_data_insert():
    ...
    insert_query = (
        "INSERT INTO moevingdb.vehicle_soc_data (vehicle_id, soc, updated_on, charge_cycles, odometer,motor_temperature, "
    ...
    update_vehicle_master_soc = "update moevingdb.TBL_VEHICLE_DETAILS set SOC = %s where FLD_RC_NUMBER = %s "
    update_vehicle_master_discharge_flag = ("update moevingdb.TBL_VEHICLE_DETAILS set FLD_IS_DEEP_DISCHARGED = 1 where "
                                            "FLD_RC_NUMBER = %s")
    reset_vehicle_deep_discharge_flag = ("update moevingdb.TBL_VEHICLE_DETAILS set FLD_IS_DEEP_DISCHARGED = 0 where "
                                         "FLD_RC_NUMBER = %s")
                ...
                if updated_on.date() == today and distance_covered > 2:
                    if vehicle_id in result_dictionary:
                        ...
                        print(
                            ...
                            "moevingdb.deep_dischage_flag for:- " + vehicle_id + ", GPS data updated at:" + str(
