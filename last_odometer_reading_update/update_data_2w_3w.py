...
can_result_2w_3w = """select  distinct on (vehicle_no) vehicle_no, tvd.fld_vehicle_id, beacon_date_time, odometer from can_data cd 
inner join tbl_vehicle_details tvd on tvd.fld_rc_number = cd.vehicle_no
where  odometer >0 and tvd.fld_vehicle_type_id in (1,2) and tvd.oem_id not in (22)
order by vehicle_no, beacon_date_time desc;
"""
cursor.execute(can_result_2w_3w)
can_rows_2w_3w = cursor.fetchall()
...
for odometer in can_rows_2w_3w:
    ...
    for vehicle in vehicle_details:
        if vehicle[0] == vehicle_id:
            ...
            mysql_conn.commit()
            print(f"Updating 2 wheeler and 3 wheeler data for {vehicle_number}.")
            vehicle_found = True
            break
    if not vehicle_found:
        mysql_cursor.execute(insert_query, (vehicle_id, vehicle_number, timestamp, odometer_reading))
        mysql_conn.commit()
        print(f"Inserting data for 2w and 3w {vehicle_number}.")
