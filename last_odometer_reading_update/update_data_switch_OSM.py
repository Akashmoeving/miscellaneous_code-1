...
can_result_switch_osm = """select  distinct on (vehicle_no) vehicle_no, tvd.fld_vehicle_id, beacon_date_time, odometer from can_data cd 
inner join tbl_vehicle_details tvd on tvd.fld_rc_number = cd.vehicle_no
where  odometer >0 and tvd.fld_vehicle_type_id in (3,2,1) and tvd.oem_id in (12,22,26,28) and cd.key_on_indicator =1
order by vehicle_no, beacon_date_time desc;
"""
cursor.execute(can_result_switch_osm)
can_rows_switch_osm = cursor.fetchall()

mysql_conn = mysql.connector.connect(host="localhost:3306", user="live_user", password="1PC^&uO$5",
                               database="mydb")
...
for odometer in can_rows_switch_osm:
    ...
    print("Vehicle details", vehicle_number)
    vehicle_found = False
    ...
    #         mysql_conn.commit()
    #         print(f"Updating switch and Omega data for {vehicle_number}.")
    #         vehicle_found = True
    #         break
    # if not vehicle_found:
    #     mysql_cursor.execute(insert_query, (vehicle_id, vehicle_number, timestamp, odometer_reading))
    #     mysql_conn.commit()
    #     print(f"Inserting data for switch and Omega {vehicle_number}.")
